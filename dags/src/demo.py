...
def load_train_files(**kwargs):
        i=0
        train_files = []
        path = os.path.join(os.path.dirname(__file__), '../data/train_set_A')
        #path = "D:/IE7374_MLOps/MLOps Project/Early_Prediction_of_Sepsis/dags/data/train_set_A"
...
def load_test_files(**kwargs):
        ...
        path = os.path.join(os.path.dirname(__file__), '../data/train_set_B')
        for f in os.listdir(path):
            if i>20:
                break
            if os.path.isfile(os.path.join(path, f)) and not f.lower().startswith('.') and f.lower().endswith('psv'):
